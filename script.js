const questions = {
    ...
    medium: [
      ...
      { question: "Pergunta 4 Modo Médio: Quando um usuário solicita a criação de uma nova equipe no Microsoft Teams, qual a abordagem correta?", options: ["Criar uma nova equipe independentemente do número de membros ou da justificativa, sem verificar a existência de equipes anteriores, e garantir que todos os dados sejam migrados sem restrições.", "Liberar a criação de novas equipes a qualquer momento, permitindo que o usuário crie equipes sem precisar justificar, ignorando a quantidade mínima de membros e deixando que qualquer pessoa, incluindo estagiários, se tornem os responsáveis.", "Não verificar a necessidade de criar uma nova equipe, simplesmente pedir para o usuário enviar um e-mail à STI, com ou sem justificativa, ignorando a quantidade de participantes ou a recomendação sobre a definição dos proprietários.", "Verificar se já existe uma equipe no setor, se sim, orientar o uso da equipe existente e, se necessário, criar uma nova equipe, desde que o número de membros seja maior que 3, e os donos sejam devidamente definidos."], correct: "Verificar se já existe uma equipe no setor, se sim, orientar o uso da equipe existente e, se necessário, criar uma nova equipe, desde que o número de membros seja maior que 3, e os donos sejam devidamente definidos." },
      { question: "Pergunta 5 Modo Médio: Quando uma equipe no Microsoft Teams atinge o limite de armazenamento, qual das ações abaixo deve ser realizada?", options: ["Verificar a lixeira, avaliar se o conteúdo pessoal está sendo armazenado, e analisar se há arquivos de áudio ou vídeo que podem ser comprimidos para reduzir o tamanho. Caso seja necessário liberar mais espaço (por exemplo, 100 GB), encaminhar o chamado para a fila de analistas, explicando as limitações do Teams para armazenamento de grandes volumes de dados e informando que o OneDrive deve ser utilizado para arquivos pessoais.", " Ignorar o limite de armazenamento e permitir que a equipe continue carregando arquivos sem nenhuma verificação, já que o espaço do Teams é ilimitado e não há necessidade de controle de conteúdo.", "Apagar aleatoriamente todos os arquivos de áudio e vídeo armazenados na equipe, sem consultar os responsáveis, sem se preocupar com o impacto na qualidade dos conteúdos ou com os requisitos de publicação, e enviar um relatório simples sem maiores verificações.", "Enviar um aviso para a equipe pedindo para ''se resolverem'', pois o problema de armazenamento não é algo que precisa de intervenção técnica. Caso não consigam, recomendar o uso de um pen drive externo para armazenar os arquivos."], correct: "Verificar a lixeira, avaliar se o conteúdo pessoal está sendo armazenado, e analisar se há arquivos de áudio ou vídeo que podem ser comprimidos para reduzir o tamanho. Caso seja necessário liberar mais espaço (por exemplo, 100 GB), encaminhar o chamado para a fila de analistas, explicando as limitações do Teams para armazenamento de grandes volumes de dados e informando que o OneDrive deve ser utilizado para arquivos pessoais." },
